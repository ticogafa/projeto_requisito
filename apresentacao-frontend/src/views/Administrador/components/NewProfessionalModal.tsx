      ...
      <div className="bg-dark-800 rounded-2xl p-8 max-w-2xl w-full border border-dark-600 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-2xl font-bold flex items-center gap-2">
            <span className="material-icons text-primary">person_add</span> Novo Profissional
          </h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input required type="text" value={cpf} onChange={e => setCpf(e.target.value)} placeholder="000.000.000-00" className="w-full bg-dark-700 border border-dark-600 rounded-lg px-3 py-2 text-white focus:border-primary focus:outline-none" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
          ...
          <div>
            ...
            <select value={senioridade} onChange={e => setSenioridade(e.target.value)} className="w-full bg-dark-700 border border-dark-600 rounded-lg px-3 py-2 text-white focus:border-primary focus:outline-none">
          ...
          <div className="grid grid-cols-2 gap-4 bg-dark-700 p-3 rounded-lg border border-dark-600">
            <div>
              ...
              <input type="time" value={inicioJornada} onChange={e => setInicioJornada(e.target.value + ':00')} className="bg-transparent text-white font-mono focus:outline-none" />
            </div>
            <div>
              <label className="block text-xs text-gray-400 mb-1">Fim Jornada</label>
              <input type="time" value={fimJornada} onChange={e => setFimJornada(e.target.value + ':00')} className="bg-transparent text-white font-mono focus:outline-none" />
            </div>
          </div>
          <div>
          ...
          <button type="submit" className="w-full bg-primary hover:bg-orange-600 text-white font-bold py-3 rounded-lg mt-4 transition">Salvar Profissional</button>
        </form>
      </div>
    </div>
  );
