...
export default function AgendamentoView() {
  ...
  // TODO: Substituir por ID real do cliente logado
  const clienteId = 1;
  ...
  const loadAgendamentos = async () => {
    setLoading(true);
    try {
      const response = await fetch(`http://localhost:8080/api/agendamentos/por-cliente?clienteId=${clienteId}`);
      if (response.ok) {
        const data = await response.json();
        const normalized = normalizeIds(data) as AgendamentoInterface[];
  ...
  const handleCancelar = async (agendamento: AgendamentoInterface) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/api/agendamentos/${agendamento.id}?clienteId=${clienteId}`,
        { method: 'DELETE' }
      );
